...
EXPECTED_WARNINGS: List[Tuple[Type[Warning], str]] = [
    ...
    (
        ...
        r"Creating an ndarray from ragged nested sequences",
    ),
    # cf. https://github.com/tensorflow/tensorflow/issues/38168
    (
        UserWarning,
        r"Converting sparse IndexedSlices.* to a dense Tensor of unknown "
        r"shape. This may consume a large amount of memory.",
    ),
    (UserWarning, r"Slot auto-fill has been removed in 3.0"),
    ...
    (DeprecationWarning, r"the imp module is deprecated in favour of importlib.*"),
    # Cannot fix this deprecation warning since we need to support two
    # numpy versions as long as we keep python 37 around
    (DeprecationWarning, r"the `interpolation=` argument to quantile was renamed"),
    ...
    (DeprecationWarning, r"the load_module"),
    (ImportWarning, r"_SixMetaPathImporter.find_spec"),
    # 3.10 specific warning: https://github.com/pytest-dev/pytest-asyncio/issues/212
    (DeprecationWarning, r"There is no current event loop"),
    # UserWarning which is always issued if the default value for
    # assistant_id key in config file is not changed
    (UserWarning, r"is missing a unique value for the 'assistant_id' mandatory key"),
    (
        DeprecationWarning,
        r"non-integer arguments to randrange\(\) have been deprecated since",
    ),
]
